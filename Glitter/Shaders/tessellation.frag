...
const float specularCof = 6.0;

layout(location = 0) out vec4 FragColor;   
layout(location = 1) out vec4 TriId; 
...
void main()
{
    vec3 lightDir = mat3(view) * normalize(worldLightDir);
    vec3 Normal = mat3(view) * normalize(Nor_FS_in);
    float diffuseFactor = max(dot(Normal, lightDir), 0.0); 
    vec3 viewDir = normalize(-Pos_FS_in);
    vec3 halfDir = normalize(lightDir + viewDir);
    float specAngle = max(dot(halfDir, Normal), 0.0);
    float specularFactor = pow(specAngle, specularCof);

    FragColor.xyz = Col_FS_in * (diffuseFactor + diffuseOffset) + specularCoeff * Spe_FS_in.xyz * specularFactor + Emi_FS_in;

    TriId.r = float(TriId_FS_in % 256) / 255.0;
    TriId.g = float(TriId_FS_in / 256) / 255.0;
    TriId.b = 0.299 * FragColor.r + 0.587 * FragColor.g + 0.114 * FragColor.b;

    vec3 fragDirection  = -viewDir;
